...
class RegisterLoginBloc extends Bloc<RegisterLoginEvent,RegisterLoginState>{
  ...
  RegisterLoginBloc({required this.registerLoginApiService,required this.secureStorageService}):super(RegisterLoginInitalState()){
on<RegisterLoginSubmitEvent>(_onRegisterLoginSubmitter);
  }
  Future _onRegisterLoginSubmitter(RegisterLoginSubmitEvent event, Emitter<RegisterLoginState>emit)async{
    try {
      final response=await registerLoginApiService.loginRegister(event.phoneNumber, event.name);
