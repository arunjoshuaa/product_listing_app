  ...
  Future verify(String phoneNumber) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {"otp": response.data['otp'], "user": response.data['user']};
      }
      throw Exception(
        "Verificaton failed with status code of${response.statusCode}-${response.statusMessage}",
      );
  ...
  Future loginRegister(String phoneno, String firstname) async {
    try {
      ...
      final response = await dio.post(
        ...
        data: {"phone_number": phoneno, "first_name": firstname},
      );
